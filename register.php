...
if (isset($_POST['signup'])) {
	$required = array('firstName', 'lastName', 'username', 'email', 'phone', 'password', 'passwordAgain', 'gender', 'month', 'day', 'year');
	foreach ($_POST as $key => $value) {
		...
		if (empty($errors['allFields'])) {
			...
			$phone = Validate::escape($_POST['phone']);
			...
			if (!Validate::validName($firstName)) {
				$errors['names'] = "Use valid names";
			} elseif (!Validate::validName($lastName)) {
				$errors['names'] = "Use valid names";
			}

			if (Validate::length($firstName, 2, 20) || Validate::length($lastName, 2, 20)) {
				$errors['names'] = "Names needs to be between 2-20 characters long";
			}

			if (Validate::length($username, 2, 10)) {
				$errors['username'] = "Username needs to be between 2-10 characters long";
			} elseif ($userObj->usernameExists($username)) {
				$errors['username'] = "Username is already taken!";
			}

			if (!empty($phone)) {
				if (!preg_match('/^[0-9]+$/', $phone)) { //  '/^[0-9]{11}+$/' /'([+]?\d{1,2}[.-\s]?)?(\d{3}[.-]?){2}\d{7}'/
					$errors['phone'] = "Only Valid numbers are allowed. Use country code without + sign";
				}
			} else {
			...
			if (!Validate::filterEmail($email)) {
				...
				$user_id = $userObj->insert('users', array('firstName' => $firstName, 'lastName' => $lastName, 'username' => $username, 'email' => $email, 'phone' => $phone, 'password' => $hash, 'birthdate' => $birthdate, 'gender' => $gender));
			...
			<div class="sign-up-inner">
				<div class="sign-up-div">
					<form method="POST">
						...
						<div>
							<h3>Phone</h3>
							<input type="tel" name="phone" placeholder="Phone" value="<?php echo (isset($phone) ? Validate::escape($phone) : ''); ?>" />
							<!-- Email Error -->
							<?php if (isset($errors['phone'])) : ?>
								<span class="error-in"><?php echo $errors['phone']; ?></span>
							<?php endif; ?>
						</div>
