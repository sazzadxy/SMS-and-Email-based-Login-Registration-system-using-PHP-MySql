...
if (isset($_POST['update'])) {
	...
	if (empty($errors['allFields'])) {
		...
		if (Validate::length($firstName, 2, 20)) {
			$errors['names'] = "Names need to be between 2-20 characters long";
		...
		if (Validate::length($username, 2, 10)) {
			$errors['username'] = "Usernames need to be between 2-10 characters long";
		} elseif ($username != $user->username && $userObj->usernameExists($username)) {
		...
		if (!Validate::filterEmail($email)) {
			...
			if (password_verify($password, $user->password)) {
				$user_id = $userObj->update('users', array('firstName' => $firstName, 'lastName' => $lastName, 'username' => $username, 'email' => $email), array('user_id' => $user_id));
...
<div class="home-nav">
	...
	<a href="<?php echo BASE_URL; ?>account/delete">Delete Profile</a>
</div>
